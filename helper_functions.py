...
def pred_and_plot_image(
    ...
    model.eval()
    with torch.inference_mode():
        target_image = target_image.unqueeze(dim=0)
    ...
    if class_names:
        title = f"Pred: {class_names[target_image_pred_label.cpu()]} | prob: {target_image_pred_prob.max().cpu()}"
    else:
        title = f"Pred: {target_image_pred_label.cpu()} | prob: {target_image_pred_prob.max().cpu()}"
